...
export default function UsersPage() {
  ...
  useEffect(() => {
    loadUsers();
  }, [currentPage, searchQuery, selectedApp?.id]);
  ...
  const loadUsers = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/partner/users?${params.toString()}`);
      if (response.ok) {
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-gray-500 mt-1">
              View referral statistics for all users {selectedApp ? `in ${selectedApp.name}` : 'across all apps'}.
            </p>
          </div>
        </div>
            ...
            <>
              <div className="divide-y divide-gray-100 dark:divide-gray-800">
                {users.map((user) => (
                  <div
                    key={user.userId}
                    onClick={() => router.push(`/dashboard/v2/users/${user.userId}`)}
