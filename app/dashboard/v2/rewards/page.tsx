...
export default function RewardsPage() {
  ...
  const updateStatus = async (rewardId: string, status: RewardStatus, extra?: { payoutReference?: string; fulfillmentType?: string; fulfillmentReference?: string }) => {
    setUpdating(true);
    try {
      const res = await fetch(`/api/partner/rewards/${rewardId}`, {
        method: 'PATCH',
                ...
                {loading ? (
                  ...
                  rewards.map((r) => (
                    <tr key={r.id} className="hover:bg-gray-50 dark:hover:bg-gray-900 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <Link href={`/dashboard/v2/users/${r.userId}`} className="font-mono text-xs text-blue-600 hover:underline" onClick={(e) => e.stopPropagation()}>
                          {r.userId}
                        </Link>
                      </td>
                      <td className="px-6 py-4">
                        <div>
                          <div className="font-medium">{r.Referral?.Campaign?.name ?? '—'}</div>
            ...
            >
              ...
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <input
                    type="text"
                    placeholder="e.g. 10% Restaurant X"
